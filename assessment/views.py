...
@api_view(["POST"])
def save_assessment_student(request):
    if request.user.is_anonymous or not request.user.role == "student":
        return Response(
            {"error": "Authentication required."},
            status=status.HTTP_401_UNAUTHORIZED,
        )
    ...
    try:
        ...
        if school.uniqueSchoolCode != "adminSchool":
            return Response(
                {"error": "You are not allowed to create assessments."},
...
@api_view(["POST"])
def generate_assessment_student(request):
    if request.user.is_anonymous or not request.user.role == "student":
        return Response(
            {"error": "Authentication required."},
            status=status.HTTP_401_UNAUTHORIZED,
...
@api_view(["POST"])
def delete_assessment_student(request):
    if request.user.is_anonymous or request.user.role != "student":
        return Response(
            {"error": "Authentication required."},
            status=status.HTTP_401_UNAUTHORIZED,
...
@api_view(["GET"])
def get_assessments_student(request):
    if request.user.is_anonymous:
        return Response(
            {"error": "Authentication required."},
            status=status.HTTP_401_UNAUTHORIZED,
    ...
    try:
        ...
        if school.uniqueSchoolCode != "adminSchool":
            return Response(
                {"error": "You are not allowed to view assessments."},
                status=status.HTTP_403_FORBIDDEN,
            )
    except AccountAffiliation.DoesNotExist:
        return Response(
